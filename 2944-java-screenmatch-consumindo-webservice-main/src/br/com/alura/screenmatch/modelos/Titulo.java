...
public class Titulo implements Comparable<Titulo> {
    ...
    public Titulo(TituloOmdb meTituloOmdb) {
        ...
        // Remover qualquer caractere não numérico do ano de lançamento
        String ano = meTituloOmdb.year().replaceAll("[^0-9]", "");
        if (ano.length() > 4) {
        ...
        // Remover qualquer caractere não numérico da duração em minutos
        String duracao = meTituloOmdb.runtime().replaceAll("[^0-9]", "");
        this.duracaoEmMinutos = Integer.valueOf(duracao);
    
        this.genero = meTituloOmdb.genre();
    }
